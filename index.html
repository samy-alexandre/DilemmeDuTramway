...
window.CONTRADICTION_RULES = [
  ...
  {
    name: "tyran_garde_secret_vs_peine_mort",
    check: (votes) => {
      const tyran = votes["lever:tyran"];
      const peineMort = votes["justice:peine_mort"];
      if (tyran === "wait" && peineMort === "wait") {
        return "Vous laissez le tramway tuer le tyran sans procès (et condamner ses otages avec lui), mais vous refusez la peine de mort encadrée par un tribunal. Votre justice est-elle un principe, ou une pulsion ?";
      }
      return null;
    }
  },
