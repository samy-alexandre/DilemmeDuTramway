...
(function() {
  ...
  // Two side-by-side option labels — used as fallback for any scene
  function optionLabels(leftLabel, rightLabel) {
    return `
      <text x="${W * 0.22}" y="${H - 18}" text-anchor="middle" font-size="11" fill="rgba(255,255,255,0.5)" letter-spacing="1" font-family="Georgia,serif">${escapeXml(leftLabel || "AGIR").toUpperCase()}</text>
      <text x="${W * 0.78}" y="${H - 18}" text-anchor="middle" font-size="11" fill="rgba(255,255,255,0.5)" letter-spacing="1" font-family="Georgia,serif">${escapeXml(rightLabel || "ATTENDRE").toUpperCase()}</text>
      <line x1="${W * 0.22 - 30}" y1="${H - 32}" x2="${W * 0.22 + 30}" y2="${H - 32}" stroke="rgba(255,255,255,0.15)" stroke-width="0.5"/>
      <line x1="${W * 0.78 - 30}" y1="${H - 32}" x2="${W * 0.78 + 30}" y2="${H - 32}" stroke="rgba(255,255,255,0.15)" stroke-width="0.5"/>
    `;
  }
  ...
  function buildTrolley(scenario) {
    return `<svg ${baseAttrs} class="scene-svg scene-trolley">
      ...
      <!-- Deviated rail (up to one person) -->
      ...
      <!-- 5 people (main rail, right side) -->
      <g id="trolley-five">
        ${Array.from({length: 5}, (_, i) => {
          const x = 460 + i * 18;
          return `<g class="figure" transform="translate(${x}, 195)">
      ...
      <!-- 1 person (deviated rail) -->
      <g id="trolley-one" transform="translate(${W-60}, 95)">
        <circle cx="0" cy="-10" r="4" fill="#e8e8e8"/>
        <rect x="-3" y="-5" width="6" height="13" rx="1" fill="#e8e8e8"/>
      </g>
  ...
  function buildBridge(scenario) {
    return `<svg ${baseAttrs} class="scene-svg scene-bridge">
      ...
      ${optionLabels("Pousser", "Ne rien faire")}
    </svg>`;
  }
  ...
  function buildMedecin(scenario) {
    return `<svg ${baseAttrs} class="scene-svg scene-medecin">
      ...
      ${optionLabels("Prélever", "Ne rien faire")}
    </svg>`;
  }
  ...
  function buildJustice(scenario) {
    return `<svg ${baseAttrs} class="scene-svg scene-justice">
      ...
      ${optionLabels("Condamner", "Acquitter")}
    </svg>`;
  }
  ...
  function buildIA(scenario) {
    return `<svg ${baseAttrs} class="scene-svg scene-ia">
      ...
      ${optionLabels("Suivre l'IA", "Refuser")}
    </svg>`;
  }
  ...
  function buildArgent(scenario) {
    return `<svg ${baseAttrs} class="scene-svg scene-argent">
      ...
      ${optionLabels("Argent", "Conscience")}
    </svg>`;
  }
  ...
  function buildMensonge(scenario) {
    return `<svg ${baseAttrs} class="scene-svg scene-mensonge">
      ...
      ${optionLabels("Dire la vérité", "Mentir")}
    </svg>`;
  }
  ...
  function buildPunition(scenario) {
    ...
    return `<svg ${baseAttrs} class="scene-svg scene-punition">
      ...
      ${optionLabels("Punir tous", "Cibler un")}
    </svg>`;
  }
  ...
  function buildAnimaux(scenario) {
    return `<svg ${baseAttrs} class="scene-svg scene-animaux">
      ...
      ${optionLabels("5 humains", "Mon animal")}
    </svg>`;
  }
